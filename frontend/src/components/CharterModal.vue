...
<template>
  <div v-if="show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg max-w-lg w-full p-6 m-4">
      <h3 class="text-lg mb-3 flex items-center gap-2"><Icon name="doc" class="ico text-accent" /> {{ scope === 'group' ? '课题组公约' : '数据集公约' }}
        <span class="tag ml-1">v{{ charter.version }}</span></h3>
